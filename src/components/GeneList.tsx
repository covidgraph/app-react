...
import { Maybe, Gene, _GeneMapsGeneSymbols } from "../generated/graphql";
...
function GeneList(props: any) {
  ...
  const getFilter = () => {
    ...
    return {
      OR: [
        {
          Full_name_from_nomenclature_authority_contains:
            filterState.searchTermFilter,
        },
  ...
  const moreInformationFullScreen = () => {
    return (
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>{detailedInformation()}</DialogContentText>
        </DialogContent>
        <DialogActions>
  ...
  const detailedInformation = () => {
    return data.Gene.filter(
      (gene: Gene) => gene.sid === currentDisplayInfo
    ).map((filteredGene: Gene) => (
      <div key={filteredGene?.sid}>
        <Divider variant="fullWidth" />
        Gene Symbols:
        {filteredGene.mapsGeneSymbols?.map(
          (geneMapsGeneSymbols: Maybe<_GeneMapsGeneSymbols>) => {
            return (
              <Tooltip
                title={
                  geneMapsGeneSymbols?.symbol?.status +
                  " from " +
                  geneMapsGeneSymbols?.source
                }
              >
        ...
        {filteredGene.Full_name_from_nomenclature_authority}
        <Divider variant="fullWidth" />
        ...
        {filteredGene.Symbol_from_nomenclature_authority}
        <Divider variant="fullWidth" />
          ...
          return (
            <div>
              {transcripts?.transcript.proteins.map((protein: any) => {
                return (
                  <div>
                    SID:{protein.protein.sid}|name:{protein.protein.name}
                  </div>
                );
              })}
            </div>
                ...
                {data.Gene.map((n: Gene) => {
                  return (
                    ...
                    >
                      ...
                      <TableCell className={classes.tableCell}>
                        {n.Full_name_from_nomenclature_authority}
                      </TableCell>
                      <TableCell className={classes.tableCell}>
                        {n.GeneID}
                      </TableCell>
                      ...
                      <TableCell className={classes.tableCell}>
                        {n.Nomenclature_status}
                      </TableCell>
                      <TableCell className={classes.tableCell}>
                        {n.Other_designations}
                      </TableCell>
                      <TableCell className={classes.tableCell}>
                        {n.Symbol}
                      </TableCell>
                      <TableCell className={classes.tableCell}>
                        {n.Symbol_from_nomenclature_authority}
                      </TableCell>
                      <TableCell className={classes.tableCell}>
                        {n.Synonyms}
                      </TableCell>
                      ...
                      <TableCell className={classes.tableCell}>
                        {n.pathways?.map((pathways: any) => {
                          return <div>{pathways?.pathway.name}</div>;
                        })}
                      </TableCell>
                      <TableCell className={classes.tableCell}>
                        {n.transcripts?.map((transcripts: any) => {
                          return (
                            <div>
                              {transcripts?.transcript.proteins.map(
                                (protein: any) => {
                                  return (
                                    <div>
                                      SID:{protein.protein.sid}|name:
